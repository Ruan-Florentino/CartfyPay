  ...
  return (
    <div className="min-h-screen bg-[#0B0B0F] pb-32 relative">
      ...
      <div className="p-6 pt-12 flex items-center justify-between sticky top-0 bg-[#0B0B0F]/80 backdrop-blur-xl z-40 border-b border-white/5 shadow-sm">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white">Meus Cursos</h1>
          <p className="text-zinc-400 text-sm mt-1 font-medium">Continue aprendendo</p>
        </div>
        <motion.button 
          whileHover={{ scale: 1.05, rotate: 5 }}
          whileTap={{ scale: 0.95 }}
          className="w-12 h-12 bg-[#111118] rounded-2xl border border-white/10 shadow-[0_0_15px_rgba(0,0,0,0.5)] flex items-center justify-center hover:bg-white/5 transition-colors text-[#FF6A00]"
        >
          <Filter size={24} />
        </motion.button>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-[#FF6A00]/20 to-[#6C2BFF]/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#111118] border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white placeholder:text-zinc-500 focus:outline-none focus:border-[#FF6A00]/50 focus:ring-1 focus:ring-[#FF6A00]/50 transition-all text-base shadow-inner"
            />
          </div>
        </motion.div>
        ...
        >
              ...
              >
                <Link href={`/cursos/${course.id}`} className="block">
                  <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="bg-[#111118] rounded-[2rem] border border-white/5 overflow-hidden group hover:border-white/10 transition-all shadow-[0_10px_30px_rgba(0,0,0,0.5)] relative"
                  >
                    {/* Hover Glow */}
                    ...
                    <div className="relative h-56 w-full overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                        <div className="w-16 h-16 bg-gradient-to-tr from-[#FF6A00] to-[#FF8C00] rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(255,106,0,0.6)] pl-1 transform group-hover:scale-110 transition-transform duration-300">
                      ...
                      <div className="absolute top-4 left-4 flex gap-2 z-20">
                        {course.badge && (
                          <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase backdrop-blur-md border flex items-center gap-1 ${getBadgeColor(course.badge)}`}>
                      ...
                      <button 
                        ...
                        className={`absolute top-4 right-4 w-8 h-8 rounded-full backdrop-blur-md flex items-center justify-center border transition-colors z-30 ${
                        course.isFavorite 
                      ...
                      <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end z-20">
                        <div className="flex gap-2">
                          <span className="bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] font-bold text-white border border-white/10 flex items-center gap-1">
                            <Clock size={12} className="text-[#FF6A00]" /> {course.duration}
                          </span>
                          <span className="bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] font-bold text-white border border-white/10 flex items-center gap-1">
                            <Star size={12} className="text-[#FFD700]" fill="currentColor" /> {course.rating}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Content */}
                    <div className="p-6 relative z-20 bg-[#111118]">
                      <h3 className="font-black text-xl text-white mb-1 line-clamp-1 group-hover:text-[#FF6A00] transition-colors">{course.title}</h3>
                      <p className="text-zinc-500 text-sm mb-5 font-medium flex items-center gap-1.5">
                      ...
                      <div className="space-y-2.5">
                        <div className="flex justify-between text-xs font-bold">
                          <span className="text-zinc-400 bg-white/5 px-2 py-1 rounded-md">{course.completed} de {course.modules} módulos</span>
                          <span className={`px-2 py-1 rounded-md flex items-center gap-1 ${course.progress === 100 ? "text-emerald-400 bg-emerald-500/10" : "text-[#6C2BFF] bg-[#6C2BFF]/10"}`}>
                            {course.progress === 100 && <CheckCircle2 size={12} />}
                            {course.progress === 100 ? "Concluído" : "Em andamento"}
                          </span>
                        </div>
                        <div className="h-2.5 w-full bg-zinc-800/50 rounded-full overflow-hidden border border-white/5 p-[1px]">
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: `${course.progress}%` }}
                            transition={{ duration: 1.5, ease: "easeOut", delay: 0.2 }}
                            className={`h-full rounded-full relative ${
                              course.progress === 100 ? "bg-gradient-to-r from-emerald-500 to-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-gradient-to-r from-[#6C2BFF] to-[#FF6A00] shadow-[0_0_10px_rgba(108,43,255,0.5)]"
                            }`}
                          >
                      ...
                      {/* Action Button */}
                      <div className={`w-full mt-6 font-black text-base py-4 rounded-2xl transition-all flex items-center justify-center gap-2 ${
                        course.progress === 100 
                          ? "bg-zinc-800 text-white group-hover:bg-zinc-700 border border-white/5" 
                          : "bg-gradient-to-r from-[#6C2BFF] to-purple-500 text-white shadow-[0_10px_30px_rgba(108,43,255,0.3)] group-hover:shadow-[0_10px_40px_rgba(108,43,255,0.5)]"
                      }`}>
          ...
          {filteredCourses.length === 0 && (
            ...
            >
              ...
              <h3 className="text-xl font-black text-white mb-2">Nenhum curso encontrado</h3>
